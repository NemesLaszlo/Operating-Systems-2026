...
int main()
{
    ...
    sprintf(pipename, "/tmp/%d", mainszalertek); // Létrehozza a pipe file nevét dinamikusan, minden futtatásnál egyedi pipe név lesz.
    ...
    printf("Szulo - Ezt olvastam a csobol: %s", pufferchar);
    close(fd);
    unlink(pipename);
